<?php
/**
 * WPTopSchool Uninstall
 *
 * Uninstalling WPTopSchool deletes user, roles, pages, tables.
 *
 */
if ( !defined( 'ABSPATH' ) ) {	exit; }
else {
	require_once(ABSPATH.'wp-admin/includes/user.php' );
	$remove_data_status	=	get_option( 'wpts_remove_data');
	if( $remove_data_status== 1 ) {
		...
		$exams_table				=	$wpdb->prefix.'wpts_exam';
		$mark_table					=	$wpdb->prefix.'wpts_mark';
		$mark_extract_table			=	$wpdb->prefix.'wpts_mark_extract';
		$mark_fields_table			=	$wpdb->prefix.'wpts_mark_fields';
		$messages_table				=	$wpdb->prefix.'wpts_messages';
		$time_table					=	$wpdb->prefix.'wpts_timetable';
		$notification_table 		=	$wpdb->prefix.'wpts_notification';
		// $hostel_table				=	$wpdb->prefix.'wpts_hostel';
		$subject_table				=	$wpdb->prefix.'wpts_subject';
		$workinghours_table			=	$wpdb->prefix.'wpts_workinghours';
		$transport_table			=	$wpdb->prefix.'wpts_transport';
		$settings_table 			=	$wpdb->prefix.'wpts_settings';
		$attendance_table 			=	$wpdb->prefix.'wpts_attendance';
		$events_table 				=	$wpdb->prefix.'wpts_events';
		$teacher_attendance_table	=	$wpdb->prefix.'wpts_teacher_attendance';
		$grade_settings_table		=	$wpdb->prefix.'wpts_grade';
		$import_history_table		=	$wpdb->prefix.'wpts_import_history';
		$wpts_fees					=	$wpdb->prefix.'wpts_fees';
		$wpts_fees_payment			=	$wpdb->prefix.'wpts_fees_payment';
		$wpts_fee_payment_history	=	$wpdb->prefix.'wpts_fee_payment_history';
		$wpts_leavedays	=	$wpdb->prefix.'wpts_leavedays';
...
if ( !defined( 'ABSPATH' ) ) {	exit; }
	...
	if( $remove_data_status== 1 ) {
		...
		$tables        =    array($teacher_table,$student_table,$class_table,$parents_table,$exams_table,$mark_table,$messages_table,$time_table,$notification_table,$hostel_table,$subject_table,$workinghours_table,$transport_table,$settings_table,$attendance_table,$events_table,$teacher_attendance_table,$grade_settings_table,$settings_table,$mark_extract_table,$mark_fields_table,$import_history_table,$leave_table,$wpts_fees, $wpts_fees_payment, $wpts_fee_payment_history);
		foreach($tables as $table) {
			$sql = "DROP TABLE IF EXISTS $table";
			$wpdb->query($sql); //Delete tables
		}
