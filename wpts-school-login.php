 ...
 */
function wpts_login_header( $title = 'Log In', $message = '', $wp_error = '' ) {
    ...
    if ( is_multisite() ) {
        $wpts_login_header_url   = network_home_url();
        $wpts_login_header_title = get_network()->site_name;
    } else {
        $wpts_login_header_url   = __( 'https://wordpress.org/' );
        $wpts_login_header_title = __( 'Powered by WordPress' );
    }
    /**
     ...
     * @since 2.1.0
     *
     * @param string $wpts_login_header_url Login header logo URL.
     */
    $wpts_login_header_url = apply_filters( 'login_headerurl', $wpts_login_header_url );
    /**
     ...
     */
    $wpts_login_header_title = apply_filters( 'login_headertitle', $wpts_login_header_title );
     ...
     * @since 4.6.0
     */
    do_action( 'wpts_login_header' );
    ?>
    <div id="login">
        <h1><a href="<?php echo esc_url( $wpts_login_header_url ); ?>" title="<?php echo esc_html( $wpts_login_header_title ); ?>" tabindex="-1"><?php bloginfo( 'name' ); ?></a></h1>
    <?php
    unset( $wpts_login_header_url, $wpts_login_header_title );
    /**
 ...
 */
function wpts_login_footer($input_id = '') {
...
case 'retrievepassword' :
    ...
    wpts_login_header(__('Lost Password'), '<p class="message">' . __('Please enter your username or email address. You will receive a link to create a new password via email.') . '</p>', $errors);
...
case 'rp' :
    ...
    if ( ( ! $errors->get_error_code() ) && isset( $_POST['pass1'] ) && !empty( $_POST['pass1'] ) ) {
        ...
        setcookie( $rp_cookie, ' ', time() - YEAR_IN_SECONDS, $rp_path, COOKIE_DOMAIN, is_ssl(), true );
        wpts_login_header( __( 'Password Reset' ), '<p class="message reset-pass">' . __( 'Your password has been reset.' ) . ' <a href="' . esc_url( wp_login_url() ) . '">' . __( 'Log in' ) . '</a></p>' );
        wpts_login_footer();
        exit;
    }
    wp_enqueue_script('utils');
    wp_enqueue_script('user-profile');
    wpts_login_header(__('Reset Password'), '<p class="message reset-pass">' . __('Enter your new password below.') . '</p>', $errors );
?>
...
case 'register' :
    ...
    wpts_login_header(__('Registration Form'), '<p class="message register">' . __('Register For This Site') . '</p>', $errors);
?>
...
<?php       exit;
    ...
    wpts_login_header(__('Log In'), '', $errors);
    if ( isset($_POST['log']) )
