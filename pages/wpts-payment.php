        ...
        <div class="wpts-row">
                    <div class="wpts-col-md-12">
                        ...
                        <div class="wpts-card-head ui-sortable-handle">
                            <h3 class="wpts-card-title"><?php esc_html_e( 'Payment Details', 'wptopschool' ); ?></h3>
                        </div>
                            <div class="wpts-card-body">
                                ...
                                <table id="class_table" class="wpts-table wpts-table-bordered wpts-table-striped" cellspacing="0" width="100%" style="width:100%">
                                    ...
                                    <tr>
                                        ...
                                        <th><?php esc_html_e( 'Amount', 'wptopschool' ); ?></th>
...
$wpts_stud_data =$wpdb->get_results("SELECT * FROM  $stable s
INNER JOIN $ctlass c
where s.wp_usr_id = '".$user_id."'");
$classid_da = maybe_unserialize($wpts_stud_data[0]->class_id);
...
$wpts_clas_data =$wpdb->get_results("SELECT * FROM  $ctlass
 where cid IN('".implode("','",$classid_da)."') and c_fee_type = 'paid'");

$proid = 0;

$wpts_fees_data = $wpdb->get_results("SELECT s.wp_usr_id,f.student_id,f.order_id FROM wp_wpts_student AS s INNER JOIN wp_wpts_fees AS f ON f.student_id =  s.wp_usr_id");


foreach($wpts_fees_data as $res){
$siid[] = $res->wp_usr_id;
}

foreach($wpts_clas_data as $clsloop){

        $wpts_teacher_data =$wpdb->get_results("SELECT * FROM  $cteacher
        where wp_usr_id = '".esc_sql($clsloop->teacher_id)."'");

        $wpts_order_data =$wpdb->get_results("SELECT c.cid,f.class_id,f.fees_amount,f.created_date FROM  wp_wpts_class as c INNER JOIN wp_wpts_fees AS f ON f.class_id = c.cid where f.class_id = '".esc_sql($clsloop->cid)."'");
        $lastKey = key(array_slice($wpts_order_data, -1, 1, true));


        if(in_array($wpts_stud_data[0]->wp_usr_id, $siid)){
            $paid = esc_html('Paid','wptopschool');
        }else{
            $paid = esc_html('Not Paid','wptopschool');
        }
        if(in_array($wpts_stud_data[0]->wp_usr_id, $siid)){
                        //   echo"<pre>";print_r($courses_value1);
                        //   $order = new WC_Order($courses_value1[$proid]);
                        //   $order_data = $order->get_data();
                          $order_date_created = $wpts_order_data[$lastKey]->created_date;
                          $newDate = date("F d, Y", strtotime($order_date_created));
                          $price = $wpts_order_data[$lastKey]->fees_amount.'.00';
                        //   $proid++;
                }
...
?>
                                    <tr>
                                        ...
                                        <td> <?php echo  esc_html($wpts_teacher_data[0]->first_name.' '. $wpts_teacher_data[0]->last_name) ;?></td>
    ...
    if($current_user_role == 'parent')
{
 ...
 $courses2 = get_user_meta($wpts_stud_data[0]->wp_usr_id, '_pay_woocommerce_enrolled_class_access_counter' );
 $courses = array_merge($courses1,$courses2);
}
else {
$courses1 = get_user_meta($wpts_stud_data[0]->parent_wp_usr_id, '_pay_woocommerce_enrolled_class_access_counter' );
...
$wpts_teacher_data =$wpdb->get_results("SELECT * FROM  $cteacher
 where wp_usr_id = '".$clsloop->teacher_id."'");
        ...
        // $pro2 = 0;
        $wpts_fees_data = $wpdb->get_results("SELECT s.wp_usr_id,f.student_id,f.class_id,f.order_id FROM wp_wpts_student AS s INNER JOIN wp_wpts_fees AS f ON f.student_id =  s.wp_usr_id where s.parent_wp_usr_id = '".$current_user->ID."'");
        foreach($wpts_fees_data as $res){
            $siid[] = $res->wp_usr_id;
            $ciid = $res->class_id;
        }
        ...
        // echo"<pre>";print_r($courses_check);
         $wpts_stud =$wpdb->get_results("SELECT * FROM  $stable s  where  s.parent_wp_usr_id = '".esc_sql($current_user->ID)."' ORDER BY s.wp_usr_id DESC");


        $wpts_tech =$wpdb->get_results("SELECT s.class_id,c.cid,c.teacher_id FROM  wp_wpts_fees AS s INNER JOIN wp_wpts_class AS c ON c.cid=s.class_id");
        foreach($wpts_tech as $key =>$value11 ){
            $tech_id = $value11->teacher_id;

        }

        foreach($wpts_stud as $value){

            if(in_array($value->wp_usr_id, $siid)){

            $wpts_cls_data =$wpdb->get_results("SELECT s.order_id,s.fees_amount,s.created_date,s.student_id,s.class_id,c.cid,c.c_name,c.teacher_id  FROM  wp_wpts_fees AS s INNER JOIN wp_wpts_class AS c ON c.cid=s.class_id where student_id = '".esc_sql($value->wp_usr_id)."' ORDER BY s.student_id DESC");

            $wpts_teacher_data =$wpdb->get_results("SELECT t.first_name,t.last_name, c.teacher_id FROM  wp_wpts_teacher as t INNER JOIN wp_wpts_class AS c ON c.teacher_id=t.wp_usr_id where wp_usr_id = '".$wpts_cls_data[0]->teacher_id."'");
            // echo"<pre>";print_r($wpts_teacher_data);
    ...
    ?>
                                    <tr>
                                        ...
                                        <td><?php
                                        $price = esc_html($wpts_cls_data[0]->fees_amount);
                                        if($price != ''){ echo get_woocommerce_currency_symbol().$price .'.00'; }else { echo '-'; }?>
                                        </td>
                                        <td>
                                        <?php
                                        // ->date('Y F j, g:i A');
                                        $order_date_created = esc_html($wpts_cls_data[0]->created_date);
                                        if($order_date_created != ''){ echo $newDate = date("F d, Y", strtotime($order_date_created)); }else { echo '-'; }?></td>
                                        <td><?php
                                            echo esc_html("Fees Paid","wptopschool");?></td>
                                        <!-- <td><a class="wpts-btn" href="<?php echo esc_url(get_permalink());?>" target="_blank">Pay Now</a></td> -->
