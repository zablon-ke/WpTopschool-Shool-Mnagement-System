	...
	if( is_user_logged_in() ) {
		...
		<div class="wpts-card">
						<div class="wpts-card-head">
							<h3 class="wpts-card-title"><?php echo apply_filters( 'wpts_student_attendance_heading_detail', esc_html__( 'Attendance Report', 'wptopschool' )); ?></h3>
							<hr>
						</div>
						<div class="wpts-card-body text-black">
							<div class="wpts-row">
								<?php
								$item =  apply_filters( 'wpts_student_attendance_title_item',esc_html("Attendance","wptopschool"));
								?>
							<div class="wpts-col-lg-5 wpts-col-md-5 wpts-col-sm-12 wpts-col-xs-12" id="AttendanceEnterForm">
							<h3 class="wpts-card-title"><?php esc_html_e( 'Attendance', 'wptopschool' ); ?></h3>
							<div class="line_box">
									<div class="wpts-form-group">
										<label for="Class">
											<?php if( isset($item['classid'])){
												esc_html_e($item['classid'],"wptopschool");
											}else{
												esc_html_e("Select Class","wptopschool");
											}
											?> </label>
											<select name="classid" id="AttendanceClass" class="wpts-form-control">
												<option value=""><?php echo esc_html("Select Class", "wptopschool");?></option>
													...
													foreach($clt as $cnm){?>
														<option value="<?php echo esc_attr($cnm->cid);?>" <?php if($cnm->cid==$selid) echo esc_html("selected","wptopschool");?>><?php echo esc_html($cnm->c_name);?></option>
													<?php }
														} else {
														$selid=0;
													$ctname=$wpdb->prefix.'wpts_class';
											...
											<span class="clserror wpts-text-red"><?php echo esc_html( 'Please11 select Class', 'wptopschool' ); ?></span>
									</div>
									<div class="wpts-form-group">
										<label for="date">	<?php if( isset($item['entry_date'])){
												esc_html_e($item['entry_date'],"wptopschool");
											}else{
												esc_html_e('Date',"wptopschool");
											}
											?></label>
										<input type="text" class="wpts-form-control select_date" id="AttendanceDate" value="<?php if(isset($_POST['entry_date'])) { echo esc_attr($_POST['entry_date']); } else { echo date('m/d/Y'); }?>" name="entry_date">
										<span class="clsdate"><?php echo esc_html( 'Please select Date', 'wptopschool' ); ?></span>
									</div>
									<div class="wpts-row wpts-text-center">
										<div class="wpts-col-sm-12">
											<div class="wpts-form-group">
												<button id="AttendanceEnter" name="attendance" class="wpts-btn wpts-btn-success"><?php echo esc_html( 'Add/Update', 'wptopschool' ); ?></button>
												<button id="Attendanceview" name="attendanceview" class="wpts-btn wpts-btn-primary"><?php echo esc_html( 'View', 'wptopschool' ); ?></button>
											</div>
										</div>
									</div>
									<div class="wpts-row">
										<div class="wpts-col-lg-12 wpts-col-md-12 wpts-col-sm-12 wpts-form-group wpts-text-red" id="wpts-error-msg" style="margin-top:0px;">
										</div>
									</div>
								</div>
							</div>
							<div class="wpts-col-lg-6 wpts-col-lg-offset-1 wpts-col-md-5 wpts-col-md-offset-1 wpts-col-sm-12 wpts-col-xs-12 AttendanceView">
								<?php
								...
								?>
								<div class="wpts-col-sm-12">
									<h3 class="wpts-card-title"><?php esc_html_e( 'View Attendance Report', 'wptopschool' ); ?></h3>
									<div class="line_box">
										<div class="box-body">
											<?php
                                                $i=0;
												...
												if(isset($attendance[$clid])){
													if($attendance[$clid]['percentage']==100)
														$css_class="wpts-progress-bar-success";
													else if($attendance[$clid]['percentage']<100 && $attendance[$clid]['percentage']>70)
														$css_class="wpts-progress-bar-warning";
													else if($attendance[$clid]['percentage']<=70)
														$css_class="wpts-progress-bar-danger";
													else
														$css_class="wpts-progress-bar-info";
												}

											?>
												<div class="wpts-progress-group">
														<span class="wpts-progress-text"><?php echo esc_html($cln);?></span>
														<span class="wpts-progress-number"><?php if(isset($attendance[$clid])){ echo esc_html($attendance[$clid]['present']); }?>/<?php echo (isset($newArr[$i]))? esc_html($newArr[$i]):'0';?></span>
														<div class="wpts-progress sm">
															<div class="wpts-progress-bar <?php echo esc_attr($css_class);?>" style="width: <?php echo (isset($attendance[$clid])) ? esc_attr($attendance[$clid]['percentage']):'0'; ?>%">
															</div>
														</div>
												</div>
											<?php
			...
			<div class="wpts-card-body">
					...
					<div class="tabSec wpts-nav-tabs-custom" id="verticalTab">
						...
						if(base64_decode(sanitize_text_field($_GET['sid'])) == $ch['sid']){ ?>
							<li class="wpts-tabing <?php echo ($i==0)?'wpts-resp-tab-active':''?>">
