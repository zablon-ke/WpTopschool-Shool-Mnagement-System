...
function wpts_AddStudent(){
	...
	if (wpts_CheckUsername($username, true) === true)
	{
		echo esc_html("Given Student User Name Already Exists!", "wptopschool");
		exit;
	}
	if (email_exists(sanitize_email($_POST['email'])))
	{
		echo esc_html("Student Email ID Already Exists!", "wptopschool");
		exit;
	}
	if (strtolower(sanitize_user($_POST['Username'])) == strtolower(sanitize_user($_POST['pUsername'])))
	{
		echo esc_html("Both USer Name Should Not Be same", "wptopschool");
		exit;
	}
	if (strtolower(sanitize_email($_POST['pEmail'])) == strtolower(sanitize_email($_POST['Email'])))
	{
		echo  esc_html("Both Email Address Should Not Be same", "wptopschool");
		exit;
	}

  if(($_POST['pEmail'] == '') AND (($_POST['pPassword'] != '') OR ($_POST['pConfirmPassword'] != '') OR ($_POST['pUsername'] != ''))){
    echo  esc_html("Please enter parent email", "wptopschool");
    exit;
  }

  if(($_POST['pUsername'] == '') AND (($_POST['pEmail'] != '') OR ($_POST['pConfirmPassword'] != '') OR ($_POST['pPassword'] != ''))){
    echo esc_html("Please enter parent username", "wptopschool");
    exit;
  }
	...
	$wpts_class_mapping_table = $wpdb->prefix . "wpts_class_mapping";
    ...
    foreach ($classarray as $id) {
      $c = esc_sql($id);
      $capacity = $wpdb->get_var("SELECT c_capacity FROM $wpts_class_table where cid='$c'");
      $class_array = $wpdb->get_results("SELECT c_name, cid FROM $wpts_class_table where cid='$c'");
      $classname = $class_array[0]->c_name;
      foreach ($class_array as $value) {
        $class_id_array = $value->cid;
      }

      $classes = $wpdb->get_results("SELECT class_id FROM $wpts_student_table");
      $classarray = array();
	...
	$email = empty($email) ? wpts_EmailGen($username) : $email;
	$userInfo = array(
		'user_login' => $username,
  ...
  $user_id = wp_insert_user($userInfo);
	...
	if (!empty($_POST['pEmail'])){
        ...
        echo esc_html("Please enter parent password", "wptopschool");
        exit;
      }
      if(($_POST['pConfirmPassword'] == '') AND (($_POST['pEmail'] != '') OR ($_POST['pPassword'] != '') OR ($_POST['pUsername'] != ''))){
        echo esc_html("Please enter parent confirm password", "wptopschool");
        exit;
      }

      if(($_POST['pConfirmPassword'] != '') AND ($_POST['pEmail'] != '') AND ($_POST['pPassword'] != '') AND ($_POST['pUsername'] != '')){
        if($_POST['p_fname'] == ''){
          echo esc_html("Please enter parent first name", "wptopschool");
          exit;
        }
      }

      if (wpts_CheckUsername(sanitize_user($_POST['pUsername']) , true) === true)
      {
				$parentMsg = esc_html("Parent UserName Already Exists", "wptopschool");
			}
			else
			{
				$parentInfo = array(
				...
				wpts_send_mail(sanitize_email($_POST['pEmail']) , 'User Registered', $msg);
	...
	if (!is_wp_error($user_id))
	{
		...
		$studenttable = array(
			...
			's_dob' => isset($_POST['s_dob']) && !empty($_POST['s_dob']) ? wpts_StoreDate(sanitize_text_field($_POST['s_dob'])) : '',
			's_doj' => isset($_POST['s_doj']) && !empty($_POST['s_doj']) ? wpts_StoreDate(sanitize_text_field($_POST['s_doj'])) : '',
		...
		wpts_send_mail($email, 'User Registered', $msg);
		$classidlist	=	array();
    ...
    $classmapping = $wpdb->prefix . "wpts_class_mapping";

		$sp_stu_ins = $wpdb->insert($wpts_student_table, $studenttable);
		$lastid = $wpdb->insert_id;
		 do_action('wpts_student_data_field',$lastid);
		foreach($c as $key => $value){
			$wpdb->insert( $classmapping, array("sid" => $lastid, "cid" => $key,"date" => wpts_StoreDate($value)));
		}
		if ($sp_stu_ins){
...
add_action('wp_ajax_check_parent_info', 'wpts_check_parent_info');
...
function wpts_check_parent_info()
...
function wpts_UpdateStudent(){
  ...
  $wpts_class_mapping_table = $wpdb->prefix . "wpts_class_mapping";
	...
	if ($studenteditprofile == 'studenteditprofile'){
		...
		$stu_upd = $wpdb->update($wpts_student_table, $studenttable, array(
	 	'wp_usr_id' => $user_id
	 ));
  ...
  if (wpts_CheckUsername($pusername, true) === true){
				echo esc_html("Given Parent User Name Already Exists!", "wptopschool");
				exit;
			}
			...
			if (email_exists(sanitize_email($_POST['pEmail'])))
			{
				...
				echo esc_html("Parent Email ID Already Exists!", "wptopschool");
        ...
        //End Update date Details onclass mapping table
				 ...
				 $sp_stu_ins = $wpdb->update($wpts_student_table, $parenttable, array(
			 	'wp_usr_id' => $user_id
			 		));
					do_action('wpts_student_data_field',$user_id);
				if (is_wp_error($user_id))
				{
	...
	$stu_upd = $wpdb->update($wpts_student_table, $parenttable, array(
...
do_action('wpts_student_data_field',$user_id);
	 ...
	 if($stu_upd){
		 ...
		 if($proversion1['status']){
		 	$wpts_c_table = $wpdb->prefix . "wpts_class";
			$wpts_u_table = $wpdb->prefix . "users";
			$useremail    = $wpdb->get_var("SELECT user_email FROM $wpts_u_table where ID = '".esc_sql($user_id)."'");
		 	$parentemail  = $wpdb->get_var("SELECT user_email FROM $wpts_u_table where ID = '".esc_sql($parent_id)."'");
			...
			foreach ($classIDArray as $id ) {
                $c = esc_sql($id);
				$clasname = $wpdb->get_var("SELECT c_name FROM $wpts_c_table where cid='$c'");
			...
			$clasnames = implode(", ",$classIDArray);
	 		if( $classnameid != "") {
				...
				wpts_send_mail($useremail, 'Your class details', $smsg);
				// Parent email
				...
				wpts_send_mail($parentemail, 'Your child class details', $pmsg);
	 		} else {
			}
		 }
	 }
   // End of the class assign email
	...
	if ($stu_upd)
	{
		do_action('wpts_UpdateStudent', $user_id, $studenttable);
	}
	if (!empty($_FILES['displaypicture']['name']))
	{
		$avatar = uploadImage('displaypicture');
...
function wpts_StudentPublicProfile()
{
	global $wpdb;
	$student_table = $wpdb->prefix . "wpts_student";
	$class_table = $wpdb->prefix . "wpts_class";
	...
	foreach ($classIDArray as $id ) {
		...
		$img_url = isset($loc_avatar['full']) && !empty($loc_avatar['full']) ? $loc_avatar['full'] : wpts_PLUGIN_URL . 'img/avatar.png';
					...
					<div class='wpts-userDetails'>
						<table class='wpts-table'>
							<tbody>
								...
								<tr>
									<td><strong>Date of Birth: </strong>" . wpts_ViewDate(esc_html($stinfo->s_dob)) . "</td>
								...
								<tr>
									<td><strong>Date of Join: </strong>" . wpts_ViewDate(esc_html($stinfo->s_doj)) . "</td>
								...
								<tr>
									<td><strong> ". apply_filters( 'wpts_parent_popupvalue', esc_html__( 'Parent Name:', 'wptopschool' ))."</strong>
										".esc_html($stinfo->p_fname. $stinfo->p_mname.  $stinfo->p_lname)."
									</td>
									<td><strong>". apply_filters( 'wpts_parent_popupvalue_gender', esc_html__( 'Parent Gender:', 'wptopschool' ))." </strong>".esc_html($stinfo->p_gender)."</td>
								</tr>
								<tr>
									<td><strong>". apply_filters( 'wpts_parent_popupvalue_email', esc_html__( 'Parent Email:', 'wptopschool' ))." </strong>".esc_html($parentEmail)."</td>
									<td><strong>". apply_filters( 'wpts_parent_popupvalue_parentprofession', esc_html__( ' Parent Profession:', 'wptopschool' ))." </strong>".esc_html($stinfo->p_profession)."</td>
								</tr>
								<tr>
	...
	echo apply_filters('wpts_student_profile', wp_kses_post($profile), intval($sid));
	wp_die();
}
...
function updateStudentClassDate($user_id){
  ...
  foreach($_POST['Classdata'] as $key => $value ){
    if($value == ''){
      $classidc_sdate[] = wpts_StoreDate('0000-00-00');
    }else{
      $classidc_sdate[] = $value;
    }
  }
  ...
  foreach($c as $key => $value){
    $wpdb->insert( $classmapping, array("sid" => $student_index, "cid" => $key,"date" => wpts_StoreDate($value)));
  }
