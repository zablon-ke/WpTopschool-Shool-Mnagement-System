...
function wpts_DeleteUser($uids = array() , $type)
{
	wpts_Authenticate();
	global $wpdb;
	$student_tbl = $wpdb->prefix . "wpts_student";
	$teacher_tbl = $wpdb->prefix . "wpts_teacher";
	$mark_tbl = $wpdb->prefix . "wpts_mark";
...
if (count($uids) > 0)
	...
	if (!empty($delid))
	{
		foreach($delid as $uid)
		{
			if ($type == 'student')
			{
				do_action('wpts_student_delete', intval($uid));
			...
			if ($type == 'teacher')
			{
				do_action('wpts_teacher_delete', intval($uid));
...
function wpts_kses_filter_allowed_html( $allowed ) {
...
function wpts_IsNameExist($table, $namecolumn, $name, $idcolumn = null, $id = null, $return = false)
{
	global $wpdb;
	...
	if (!empty($sql))
	{
		...
		else echo esc_html("true", "wptopschool");
	}

	else
	{
		if ($return) return false;
		else echo esc_html("false", "wptopschool");
	}
	wp_die();
}
...
function wpts_IsMarkEntered($classid, $subjectid, $examid)
{
	global $wpdb;
	$mark_tbl = $wpdb->prefix . "wpts_mark";
...
function wpts_GetMarks($classid, $subjectid, $examid)
{
	global $wpdb;
	$mtable = $wpdb->prefix . "wpts_mark";
	...
	foreach($input as $value => $rule)
	{
		...
		foreach($rules as $rl)
		{
			...
			case 'email':
				...
				break;
			case 'unique':
				if (wpts_CheckUsername($value, true))
				{
					array_push($error, 'Please enter valid email address');
				}
				break;
...
// function wpts_UploadPhoto()
// {
// 	$sid = sanitize_text_field($_POST['sid']);
// 	$gallery_path = WPTS_PLUGIN_PATH . 'uploads/gallery/' . $sid;
...
// 	wp_die();
// }
// function wpts_DeletePhoto()
// {
// 	wpts_Authenticate();
// 	$iname = sanitize_text_field($_POST['iname']);
// 	$sid = sanitize_text_field($_POST['sid']);
// 	$gallery_file = WPTS_PLUGIN_PATH . 'uploads/gallery/' . $sid . '/' . $iname;
...
// 	wp_die();
// }
function wpts_BulkDelete()
{
	$uids = explode(',', sanitize_text_field($_POST['UID']));
	$type = sanitize_text_field($_POST['type']);
	if (wpts_DeleteUser($uids, $type))
	{
		echo esc_html("success", "wptopschool");
	}
	else
	{
		echo esc_html("failed", "wptopschool");
	}
	wp_die();
}
/* Delete Student */
function wpts_DeleteStudent()
{
	wpts_Authenticate();
	global $wpdb;
	$student_tbl = $wpdb->prefix . "wpts_student";
	...
	do_action('wpts_student_delete', intval($sid));
	...
	if ($delstu) echo esc_html("success", "wptopschool");
	else echo esc_html("Something went wrong!", "wptopschool");
	wp_die();
}
/* Delete Teacher */
function wpts_DeleteTeacher()
{
	wpts_Authenticate();
	global $wpdb;
	$teacher_tbl = $wpdb->prefix . "wpts_teacher";
	...
	do_action('wpts_teacher_delete', intval($tid));
	$wpdb->delete( $usertable, array( 'ID' => $result['wp_usr_id'] ) );
	$deltec = $wpdb->delete($teacher_tbl, array(
		'tid' => $tid
	));
	if ($deltec) echo esc_html("success", "wptopschool");
	else echo esc_html("Something went wrong!", "wptopschool");
	wp_die();
}
/* Delete imported User*/
function wpts_UndoImport()
{
	wpts_Authenticate();
	global $wpdb;
	$id = intval($_POST['id']);
	$importtable = $wpdb->prefix . "wpts_import_history";
	$usertable = $wpdb->prefix . "users";
	...
	if ($type == '1')
	{
		...
		$user_del = $wpdb->delete($usertable, array('ID' => $value));
			...
			$wpts_del = $wpdb->delete($studenttable, array('wp_usr_id' => $value));
	}
	}
	else if ($type == '2')
	{
		$teachertable = $wpdb->prefix . "wpts_teacher";
		foreach($imported_array as $value)
		{
			$user_del = $wpdb->delete($usertable, array(
				'ID' => $value
			));
			$wpts_del = $wpdb->delete($teachertable, array(
				'wp_usr_id' => $value
			));
		}
	}
	...
	if ($type == '4')
	{
		...
		foreach($imported_array as $value)
		{
			...
			$wpts_del = $wpdb->delete($marktable, array(
				'mid' => $value
			));
		}
	}
	...
	if (($user_del) && ($wpts_del))
	{
        echo sprintf( __( 'Imported  %s rows are removed successfully!!', 'wptopschool' ), $count );
		// echo esc_html("Imported " . $count . " rows are removed successfully!!", "wptopschool");
	}
	else
	{
		echo esc_html("Success.. But something wrong because some rows may be deleted previously..", "wptopschool");
	}
	wp_die();
}
function wpts_ParentPublicProfile($pid = '', $button = 0)
{
	global $wpdb;
	$student_table = $wpdb->prefix . "wpts_student";
	$users_table = $wpdb->prefix . "users";
	...
	echo apply_filters('wpts_parent_profile', $profile, intval($pid));
	wp_die();
}
/****************************** Class Functions ******************************/
function wpts_AddClass()
{
	...
	$start_date = wpts_StoreDate(sanitize_text_field($_POST['Sdate']));
	$end_date = wpts_StoreDate(sanitize_text_field($_POST['Edate']));
	$classfeetype = sanitize_text_field($_POST['classfeetype']);
	if (wpts_IsNameExist($wpts_class_table, 'c_name', $class_name, '', '', true) == true)
	{
		if (isset($_POST['add_from']))
		{
			$response['statuscode'] = 0;
			$response['html'] = '';
			$response['msg'] = esc_html( 'Class Name Already Exists!' ,'wptopschool');
			echo json_encode($response);
		}
		else
		{
			echo esc_html( 'Class name exists!' ,'wptopschool');
		}
		exit;
	}
	else
	{
		...
		$wpts_class_ins = $wpdb->insert($wpts_class_table, $class_data);
		if ($wpts_class_ins)
		{
			do_action('wpts_class_created', $wpdb->insert_id, $class_data);
		}
		if (isset($_POST['add_from']) && sanitize_text_field($_POST['add_from']) == 'student')
		{
			$response['statuscode'] = 0;
			$response['html'] = '';
			$response['msg'] = esc_html( 'Error in adding class, Try again later..' ,'wptopschool');
			if ($wpts_class_ins && !empty($wpdb->insert_id))
			{
				$response['statuscode'] = 1;
				$response['html'] = '<option value="' . esc_attr($wpdb->insert_id) . '">' . esc_html($class_name) . '</option>';
				$response['msg'] = esc_html( 'Class Added Successfully' ,'wptopschool');
			}
			echo json_encode($response);
		}
		else
		{
			echo $status = $wpts_class_ins ? esc_html( 'inserted' ,'wptopschool') : esc_html( 'error' ,'wptopschool');
		}
	}
	wp_die();
}
function wpts_UpdateClass()
{
	if (!isset($_POST['caction_nonce']) || !wp_verify_nonce(sanitize_text_field($_POST['caction_nonce']) , 'ClassAction'))
	{
		echo esc_html( 'Unauthorized Submission' ,'wptopschool');
	...
	if (wpts_IsNameExist($wpts_class_table, 'c_name', $class_name, 'cid', $cid, true) == true)
	{
		echo esc_html( 'Class name Exists' ,'wptopschool');
		exit;
	}
	else
	{
		...
		$wpts_class_upd = $wpdb->update($wpts_class_table, $class_data, array(
			'cid' => $cid
		));

		if ($wpts_class_upd)
		{
			do_action('wpts_class_updated', intval($cid) , $class_data);
		}
		echo esc_html( 'updated' ,'wptopschool');
	}
	wp_die();
}
function wpts_GetClass()
{
	global $wpdb;
	...
	$clinfo['c_sdate'] = wpts_ViewDate($clinfo['c_sdate']);
	$clinfo['c_edate'] = wpts_ViewDate($clinfo['c_edate']);
	if (!empty($clinfo)) echo json_encode($clinfo);
	else echo esc_html( 'false' ,'wptopschool');
	wp_die();
}
function wpts_Updateregisterdeactive()
{
	wpts_Authenticate();
	global $wpdb;
	 $temp_tbl = $wpdb->prefix . "wpts_temp";
	$cid = intval($_POST['cid']);
	$temp_data = array(
		't_active' => '0'
	);
	...
	wpts_send_mail($userdetails['t_email'], 'Registration Denied', $msg);
	}
	else
	{
	echo esc_html( 'Something went wrong!' ,'wptopschool');
	}
	wp_die();
}
function  wpts_Updateregisteractive()
{

	wpts_Authenticate();
	global $wpdb;
	 $temp_tbl = $wpdb->prefix . "wpts_temp";
	 $wpts_student_user = $wpdb->prefix . "users";
		...
		if($userdetails['t_type'] == "student"){
			...
			$user_id = wp_insert_user($userInfo);
			 	...
			 	if (!is_wp_error($user_id))
				{
					...
					$msg.= 'Regards,<br />' . get_bloginfo('name');
					wpts_send_mail($userdetails['t_email'], 'Student Registration Confirmation', $msg);
					$sp_stu_ins = $wpdb->insert($wpts_student_table, $studenttable);
					if ($sp_stu_ins)
					{
						do_action('wpts_student_created', $user_id, $studenttable);
					}
					// send registration mail
					//wpts_send_user_register_mail($userInfo, $user_id);
					$msg = $sp_stu_ins ? esc_html( 'success' ,'wptopschool') : esc_html( 'Oops! Something went wrong try again.!' ,'wptopschool');


					$delrequest = $wpdb->delete($temp_tbl, array(
						't_id' => $cid
					));
					if ($delrequest) echo esc_html( 'success' ,'wptopschool');
					else echo esc_html( 'Something went wrong!' ,'wptopschool');
				}
				else
				if (is_wp_error($user_id))
		...
		elseif($userdetails['t_type'] == "teacher"){
			...
			$user_id = wp_insert_user($userInfo);
			 	...
			 	if (!is_wp_error($user_id))
				{
					...
					$msg.= 'Please<a href="' . esc_url(site_url() . '/sch-dashboard').'">click here </a>to loggin.<br /><br />';
					$msg.= 'Regards,<br />' . get_bloginfo('name');
					wpts_send_mail($userdetails['t_email'], 'Teacher Registration Confirmation', $msg);
					$sp_stu_ins = $wpdb->insert($wpts_teacher_table, $teachertable);
					if ($sp_stu_ins)
					{
						do_action('wpts_teacher_created', $user_id, $teachertable);
					}
					// send registration mail
					//wpts_send_user_register_mail($userInfo, $user_id);
					$msg = $sp_stu_ins ? esc_html( 'success' ,'wptopschool') : esc_html( 'Oops! Something went wrong try again.' ,'wptopschool');
...
function wpts_bulkdisaproverequest()
{
	wpts_Authenticate();
	global $wpdb;
	 $temp_tbl = $wpdb->prefix . "wpts_temp";
	...
	if ($delcl) {echo esc_html( 'success' ,'wptopschool');}

	wp_die();
}

function wpts_bulkaproverequest()
{
	wpts_Authenticate();
	global $wpdb;
	 $temp_tbl = $wpdb->prefix . "wpts_temp";
...
function wpts_DeleteClass()
{
	wpts_Authenticate();
	global $wpdb;
	$class_tbl = $wpdb->prefix . "wpts_class";
	$cid = intval($_POST['cid']);
	do_action('wpts_class_delete', intval($cid));
...
function wpts_GetClassName($class_id)
{
	global $wpdb;
	$class_tbl = $wpdb->prefix . "wpts_class";
...
function wpts_GetClassYear()
{
	global $wpdb;
	$class_id = intval($_POST['cid']);
	$class_tbl = $wpdb->prefix . "wpts_class";
	$cl_year = $wpdb->get_row("select c_sdate,c_edate from $class_tbl where cid='".esc_sql($class_id)."'", ARRAY_A);
	$cl_year['c_sdate'] = wpts_ViewDate($cl_year['c_sdate']);
	$cl_year['c_edate'] = wpts_ViewDate($cl_year['c_edate']);
	echo json_encode($cl_year);
	wp_die();
}
/* Exam Functions */
function wpts_AddExam()
{
	wpts_Authenticate();
	global $wpdb;
	$wpts_exam_table = $wpdb->prefix . "wpts_exam";
	...
	if (wpts_IsNameExist($wpts_exam_table, 'e_name', $exam_name, "", "", true) == true)
	{
		echo esc_html( 'Name Exists!' ,'wptopschool');
	}
	else
	{
		$exam_data = array(
			...
			'e_s_date' => wpts_StoreDate($exam_start) ,
			'e_e_date' => wpts_StoreDate($exam_end)
		);
		$wpts_exam_ins = $wpdb->insert($wpts_exam_table, $exam_data);
		if ($wpts_exam_ins)
		{
			do_action('wpts_exam_created', $wpdb->insert_id, $exam_data);
		}
		if ($wpts_exam_ins) echo "success";
		else echo esc_html( 'error' ,'wptopschool');
	}
	wp_die();
}
function wpts_UpdateExam()
{
	wpts_Authenticate();
	global $wpdb;
	$wpts_exam_table = $wpdb->prefix . "wpts_exam";
	...
	$wpts_exam_ins = $wpdb->update($wpts_exam_table, $exam_data, array(
		'eid' => $eid
	));
	if ($wpts_exam_ins)
	{
		do_action('wpts_exam_updated', intval($eid) , $exam_data);
	}
	if ($wpts_exam_ins)
	{
		echo esc_html( 'updated' ,'wptopschool');
	...
	{
		echo esc_html( 'error' ,'wptopschool');
	}
	wp_die();
}
function wpts_DeleteExam()
{
	wpts_Authenticate();
	global $wpdb;
	$eid = intval($_POST['eid']);
	$exam_tbl = $wpdb->prefix . "wpts_exam";
	do_action('wpts_exam_delete', intval($eid));
	$exam_del = $wpdb->delete($exam_tbl, array(
		'eid' => $eid
	));
	if ($exam_del) echo esc_html( 'deleted' ,'wptopschool');
	else echo  esc_html( 'Not deleted! Pls try again.' ,'wptopschool');
	wp_die();
}
function wpts_ExamInfo()
{
	global $wpdb;
	$etable = $wpdb->prefix . "wpts_exam";
	$eid = sanitize_text_field($_POST['eid']);
	$exinfo = $wpdb->get_row("select * from $etable where eid='".esc_sql($eid)."'");
	$stable = $wpdb->prefix . "wpts_subject";
	$html = '';
	if (!empty($exinfo))
	{
		$exinfo->e_s_date = wpts_ViewDate($exinfo->e_s_date);
		$exinfo->e_e_date = wpts_ViewDate($exinfo->e_e_date);
		$subject_list = explode(",", $exinfo->subject_id);
		$wpts_subjects = $wpdb->get_results("select * from $stable where class_id=".esc_sql($exinfo->classid)."");
		foreach($wpts_subjects as $subjectlist)
		{
			$checked = in_array($subjectlist->id, $subject_list) ? 'checked=checked' : '';
			$html.= '<input ' . $checked . ' type="checkbox" name="subjectid[]" value="' . esc_attr($subjectlist->id) . '" class="exam-subjects wpts-checkbox" id="subject-' . esc_attr($subjectlist->id) . '"><label for="subject-' . esc_attr($subjectlist->id) . '">' . esc_html($subjectlist->sub_name) . '</label>';
...
function wpts_StudentCount()
{
	...
	if ($count == '0')
	{
		...
		if ($cl_del) echo  esc_html( 'deleted' ,'wptopschool');
		else echo esc_html( 'Something went wrong! Try again..' ,'wptopschool');
	}
	else
	{
        echo sprintf( __( 'There are %s students associated with this class.', 'wptopschool' ), $count );
		// echo "There are " . esc_html($count) . " students associated with this class.";
	}
	wp_die();
}
/**************************** Attendance Functions ***********************/
function wpts_getStudentsList()
{
	global $wpdb;
	$classid = intval($_POST['classid']);
	$entry_date = date('Y-m-d', strtotime(sanitize_text_field($_POST['date'])));
	$show_date = wpts_ViewDate($entry_date);
	...
	if (!empty($startdate) && !empty($enddate))
	{
		...
		{
			$msg = __(sprintf('You have selected wrong date, your class startdate is %s and enddate %s', $check_date->c_sdate, $check_date->c_edate) , 'wptopschool');
			$response['status'] = 0;
			$response['msg'] = $msg;
	...
	$ex_absents = array();
	$title = __('New Attendance Entry', 'wptopschool');
	$warning = $nil = '';
	if ($check_attend)
	{
		$title = __('Update Attendance Entry', 'wptopschool');
		$warning = __('Already attendance were entered!', 'wptopschool');
	...
	$content = '<h3 class="wpts-card-title">' . $title . '</h3>
                    ...
                    <form name="AttendanceEntryForm" id="AttendanceEntryForm" method="post" class="form-horizontal">
                        ...
                        <div>
                            ...
                            <input type="checkbox" class="checkAll ccheckbox wpts-checkbox" ' . esc_html($nil) . ' name="Nil" value="Nil"> <span class="wpts-text-green MRTen" style="
                            ...
                            <table class="wpts-table wpts-table-bordered" id="addAttendanceTable" cellspacing="0" width="100%" style="width:100%">
                                <thead>
                                    <tr><td colspan="5">
									<span class="pull-left">Class: <span class="wpts-f500">' . esc_html($classname) . '</span></span><span class="pull-right">Date: <span class="wpts-f500">' . esc_html($show_date) . '</span></span></td>
									</tr>
                                    <tr>
										<th>#</th>
										<th>Roll No.</th>
								...
								foreach($stl as $sid)
								{
									...
									$content.= '<tr>
												...
												<td><input type="checkbox" ' . $checked . ' class="ccheckbox wpts-checkbox" name="absent[]" value="' . esc_attr($st->wp_usr_id) . '"> Absent </td>
												<td><input type="text"  name="reason[' . $st->wp_usr_id . ']" value="' .esc_attr(stripslashes($ex_absents[$st->wp_usr_id])) . '" class="wpts-form-control"></td>
											</tr>';
									$sno++;
								}
							$content.=
                    ...
                    <div class="box-footer">
                        <div class="wpts-form-group">
                            <button id="AttendanceSubmit" class="wpts-btn wpts-btn-success">Submit</button>
                        </div>
                        </div>
                    </form>
                </div>
...
function wpts_AttendanceEntry()
{
	...
	$att_table = $wpdb->prefix . "wpts_attendance";
	$stud_table = $wpdb->prefix . "wpts_student";
	$class_table = $wpdb->prefix . "wpts_class";
	...
	else
	{
		...
		foreach($abs as $stid)
		{
			...
			if (isset($wpts_settings_data['absent_sms_alert']) && $wpts_settings_data['absent_sms_alert'] == 1)
			{
			//parent absent notification enable
				...
				if (isset($studInfo->s_phone) && !empty($studInfo->s_phone))
				{
					...
					$status = apply_filters('wpts_send_notification_msg', false, $studInfo->s_phone, $absentreason);
					} else {
					$status = apply_filters('wpts_send_notification_msg_twilio', false, $studInfo->s_phone, wp_kses_post($absentreason));
				}
				}
			}
		}
	...
	{
		$msg = esc_html("error","wptopschool");
	}
	echo wp_kses_post($msg);
	wp_die();
}
function wpts_DeleteAttendance()
{
	wpts_Authenticate();
	global $wpdb;
...
function wpts_WorkingDays($class)
{
	global $wpdb;
	$class_table = $wpdb->prefix . "wpts_class";
...
function wpts_WorkingDates($start_date, $end_date, $class)
{
	global $wpdb;
	$ignore = array();
...
function wpts_leaveDates($strDateFrom, $strDateTo, $weeklyOff = array(
	'Sunday'
))
{
	$aryRange = array();
...
function wpts_GetAttReport()
{
	wpts_Authenticate();
	echo wpts_AttReport(intval($_POST['student_id']) , 0);
}
function wpts_AttReport($st_id, $close = 1)
{
	global $wpdb;
	$att_table = $wpdb->prefix . "wpts_attendance";
	$st_table = $wpdb->prefix . "wpts_student";
	$class_table = $wpdb->prefix . "wpts_class";
	$ser = '%' . $st_id . '%';
...
// print_r($classname_array);die();
	...
	$stinfo->c_edate = wpts_ViewDate($stinfo->c_edate);
	$stinfo->c_sdate = wpts_ViewDate($stinfo->c_sdate);
	...
	$content = "<div class='wpts-panel-body'>
					<div class='wpts-userpic'>
						 <img src=".esc_url($img_url)." height='150px' width='150px' class='img img-circle'/>
					</div>
					<div class='wpts-userDetails'>
...
function wpts_GetAbsentDates()
{
	...
	{
		$show_date = wpts_ViewDate($abd['date']);
...
function wpts_AddSubject()
{
		...
		for ($i = 0; $i < $n; $i++)
		{
			if ($subjects[$i] != '')
			{
				...
				if (array_search($sub_name, $class_subjects) === false)
				{
					...
					else $response_msg = esc_html( 'Subject Code Already Assigned to another subject' ,'wptopschool');
				}
				else
				{
					$response_msg = esc_html( 'Subject Name Already Exists.' ,'wptopschool');
					break;
				}
			}
		}
	...
	{
		echo esc_html( 'Subjects Empty!' ,'wptopschool');
		wp_die();
	}
	if (count($subj_array) > 0 && empty($response_msg))
	{
		foreach($subj_array as $sub_ent)
		{
			...
			do_action('wpts_subject_created', $wpdb->insert_id, $sub_ent);
		}
...
function wpts_UpdateSubject()
{
	wpts_Authenticate();

	global $wpdb;
	$sub_table = $wpdb->prefix . "wpts_subject";
	...
	{
		echo esc_html( 'Subject name exists!' ,'wptopschool');
	}
	else
	{
		$subject_data = array(
		...
		if ($sub_upd)
		{
			do_action('wpts_subject_updated', intval($srid) , $subject_data);
		}
		if ($sub_upd) echo esc_html( 'updated' ,'wptopschool');
		else echo  esc_html( 'No change found' ,'wptopschool');
	}
	wp_die();
}
function wpts_DeleteSubject()
{
	wpts_Authenticate();
	global $wpdb;
	$sub_table = $wpdb->prefix . "wpts_subject";
	$subid = intval($_POST['sid']);
	do_action('wpts_subject_delete', intval($subid));
	$sub_del = $wpdb->delete($sub_table, array(
		'id' => $subid
	));
	if ($sub_del)
	{
		echo esc_html( 'deleted' ,'wptopschool');
	}
	else
	{
		echo esc_html( 'failed' ,'wptopschool');
...
function wpts_GeneralSubjectEntry()
{
	global $wpdb;
...
function wpts_GensubjectEdit()
{
	...
	if ($sub_upd)
	{
		echo esc_html( 'update' ,'wptopschool');
	}
	else
	{
		echo esc_html( 'fail' ,'wptopschool');
	}
	wp_die();
}
/******************* Time Table ************************/
function wpts_SaveTimetable()
{
	wpts_Authenticate();
	$cid = intval($_POST['cid']);
	$tid = intval($_POST['tid']);
	...
	if (count($check_entry) > 0)
	{
		...
		if ($upd) echo esc_html( 'updated' ,'wptopschool');
		else echo esc_html( 'fail' ,'wptopschool');
	}
	else
	{
		...
		if ($ins) echo esc_html( 'true' ,'wptopschool');
		else echo  esc_html( 'false' ,'wptopschool');
	}
	wp_die();
}
function wpts_DeleteTimetable()
{
	wpts_Authenticate();
	global $wpdb;
	$ttable = $wpdb->prefix . "wpts_timetable";
	$cid = intval($_POST['cid']);
...
/*********** Remove class ************/
function wpts_DeleteTimetablesloat()
{
	wpts_Authenticate();
	global $wpdb;
	$ttable = $wpdb->prefix . "wpts_timetable";
...
function wpts_AddMark(){
	...
	$mark_table		=	$wpdb->prefix."wpts_mark";
	$exmark_table	=	$wpdb->prefix."wpts_mark_extract";
	$marklimit		=	isset( $wpts_settings_data['max_marks'] ) ? $wpts_settings_data['max_marks'] : 0;
	$msg = '';
	if( wpts_IsMarkEntered($stclass,$stsubject,$stexam) ) {
		if( isset($_POST['update']) && $_POST['update']=='true' ) {
			foreach($marks as $key=>$mark) {
				//foreach($remarks as $key=>$remarks) {
				if( $marklimit> 0 && $mark[0]>$marklimit)
					$msg = esc_html( 'Some marks couldn\'t be enterted, Marks limit exceeds' ,'wptopschool');
				else
...
function wpts_MarkReport($st_id, $c_id){
	global $wpdb;

	$mark_table = $wpdb->prefix . "wpts_mark";
	$exam_table = $wpdb->prefix . "wpts_exam";
	$subject_table = $wpdb->prefix . "wpts_subject";
	$extra_marks_table = $wpdb->prefix . "wpts_mark_extract";
	$extra_fields = $wpdb->prefix . "wpts_mark_fields";
	$marks = array();
	$prev_id = $content = '';
	...
	if (count($marks) > 0)
	{
		...
		{
			$i = 1;
			$content.= '<div class="wpts-table-responsive"><table class="wpts-table table-striped table-bordered">
	...
	{
		$content = "<label class='wpts-text-red'>No Marks available to show!</label>";
	}
	// echo $content;
     $allowed_tags = wp_kses_allowed_html('post');
    echo wp_kses(stripslashes_deep($content), $allowed_tags);
...
function wpts_GenSetting()
{
	...
	{
		$check_sett = $wpdb->get_row("Select * from $wpts_settings_table where option_name='".esc_sql($key)."'");
		if (empty($check_sett))
		{
			...
			$wpts_settings_ins = $wpdb->insert($wpts_settings_table, $settings_table_data);
		}
		else
		{
			...
			$wpts_settings_upd = $wpdb->update($wpts_settings_table, $settings_table_update_data, array(
				'option_name' => $key
			));
		}
	}
$optionvalues = array_filter($option_value);
	...
	{
		echo esc_html( 'All Fields are blank, Please insert values...', 'wptopschool' );
	}
	else
	{
		echo esc_html( 'success', 'wptopschool' );
...
function wpts_GenSettingsocial()
{
	wpts_Authenticate();
	global $wpdb;
	$wpts_settings_table = $wpdb->prefix . "wpts_settings";
	$_POST['type']= esc_html( 'social', 'wptopschool' );
...
function wpts_GenSettinglicensing()
{
	wpts_Authenticate();
	global $wpdb;
	$wpts_settings_table = $wpdb->prefix . "wpts_settings";
...
function wpts_GenSettingsms()
{
	wpts_Authenticate();
	global $wpdb;
	$wpts_settings_table = $wpdb->prefix . "wpts_settings";
	 $_POST['type']= esc_html( 'sms', 'wptopschool' );
...
function wpts_ManageGrade()
{
	wpts_Authenticate();
	global $wpdb;
	$grade_table = $wpdb->prefix . "wpts_grade";
...
function wpts_AddSubField()
{
	...
	wpts_Authenticate();
	global $wpdb;
	$fields_tbl = $wpdb->prefix . "wpts_mark_fields";
	$subject_id = intval($_POST['SubjectID']);
	$field = sanitize_text_field($_POST['FieldName']);
	if ($subject_id == '' || $field == '')
	{
		echo  esc_html( 'Check all fields are entered!', 'wptopschool' );
		exit;
	}
	...
	{
		echo  esc_html( 'Field already exists!', 'wptopschool' );
	}
	if ($ins) echo esc_html( 'success', 'wptopschool' );
	else echo  esc_html( 'Fields not saved! Pls try again', 'wptopschool' );
	wp_die();
}
function wpts_DeleteSubField()
{
	wpts_Authenticate();
	global $wpdb;
	$fields_tbl = $wpdb->prefix . "wpts_mark_fields";
	...
	if ($sfdel) echo esc_html( 'success', 'wptopschool' );
	else echo esc_html( 'Something went wrong!', 'wptopschool' );
	wp_die();
}
function wpts_UpdateSubField()
{
	wpts_Authenticate();
	global $wpdb;
	$fields_tbl = $wpdb->prefix . "wpts_mark_fields";
	...
	if ($field == "" || $sfid == 0)
	{
		echo esc_html( 'Check field name and field id!', 'wptopschool' );
	}
	else
	{
...
/********* Notify Function ***********/
function wpts_Notify()
{
	include_once ('wpts-notify.php');
}
/************ Event Functions **********/
function wpts_ListEvent()
{
	global $wpdb, $current_user;
	$start = sanitize_text_field($_POST['start']);
	$end = sanitize_text_field($_POST['end']);
	$event_table = $wpdb->prefix . "wpts_events";
...
function wpts_AddEvent()
{
	...
	$sdate = wpts_StoreDate(sanitize_text_field($_POST['sdate']));
	$edate = wpts_StoreDate(sanitize_text_field($_POST['edate']));
	$start = $sdate . ' ' . $stime;
	$end = $edate . ' ' . $etime;
	$event_data = array(
		'start' => $start,
	...
	if ($event_status)
	{
		do_action('wpts_event_created', $wpdb->insert_id, $event_data);
	}
	echo  esc_html( 'success', 'wptopschool' );
	wp_die();
}
function wpts_UpdateEvent()
{
	wpts_Authenticate();
	global $wpdb;
	$event_table = $wpdb->prefix . "wpts_events";
	if (isset($_POST['evid']) && sanitize_text_field($_POST['evid']) != '')
	{
		...
		if ($event_status)
		{
			do_action('wpts_event_updated', intval($evid) , $event_data);
		}
	}
	echo esc_html( 'success', 'wptopschool' );
	wp_die();
}
function wpts_DeleteEvent()
{
	wpts_Authenticate();
	global $wpdb;
	$event_table = $wpdb->prefix . "wpts_events";
	$evid = intval($_POST['evid']);
	do_action('wpts_event_delete', intval($evid));
...
/**************** Leave Calendar ********************/
function wpts_AddLeaveDay()
{
	wpts_Authenticate();
	global $wpdb;
	$leave_table = $wpdb->prefix . "wpts_leavedays";
	$sdate = wpts_StoreDate(sanitize_text_field($_POST['spls']));
	$edate = wpts_StoreDate(sanitize_text_field($_POST['sple']));
	...
	if ($sdate == '')
	{
		echo  esc_html( 'date missing', 'wptopschool' );
		wp_die();
	}
	else
	if (!is_numeric($class_id))
	{
		echo  esc_html( 'Invalid class id', 'wptopschool' );
		wp_die();
	}
	...
	if ($insd) echo esc_html( 'success', 'wptopschool' );
	else echo esc_html( 'Not inserted! Date may exist, pls check', 'wptopschool' );
	wp_die();
}
function wpts_GetLeaveDays()
{
	global $wpdb, $current_user;
	$cid = intval($_POST['cid']);
	$leave_table = $wpdb->prefix . "wpts_leavedays";
			...
			<div class='wpts-col-xs-12 wpts-col-sm-12 wpts-col-md-12 wpts-col-lg-12' >
			  <div class='wpts-panel wpts-panel-info'>
				<div class='wpts-panel-heading'>
				  <h3 class='wpts-panel-title textleft'>Leave Dates</h3>
				</div>
				<div class='wpts-panel-body'>
				<div class='wpts-userDetails'>
					<table class='wpts-table table-user-information'>
					<thead>
					<tr>
						<th>#</th>
						<th>Date</th>
						...
						{
							$date = wpts_ViewDate($lday->leave_date);
							echo "<tr>
									<td>".esc_html($sno)."</td>
									<td>".esc_html($date)."</td>
...
function wpts_DeleteLeave()
{
	...
	if ($ldel) echo   esc_html( 'success', 'wptopschool' );
	else echo   esc_html( 'fail', 'wptopschool' );
	wp_die();
}
/**************** Transport Functions ***************/
function wpts_FormValidation($pValues, $rFields)
{
	$error = TRUE;
...
function wpts_AddTransport()
{
	if ($_SERVER['REQUEST_METHOD'] === 'POST')
	{
		...
		$validation = wpts_FormValidation($_POST, ['VhName', 'VhNumb']);
		if ($validation === TRUE)
		{
			...
			if ($ins) echo  esc_html( 'success', 'wptopschool' );
			else echo  esc_html( 'Data not saved. Something went wrong', 'wptopschool' );
		}
		else
		{
	...
	else
	{
		...
		$pl = "";
		$item =  apply_filters( 'wpts_subject_title_item',array());
		if(isset($item['VhName'])){
			$vname = esc_html($item['VhName'],"wptopschool");
		}else{
			$vname = esc_html("Vehicle Name","wptopschool");

		}

		if(isset($item['VhNumb'])){
			$VhNumb = esc_html($item['VhNumb'],"wptopschool");
		}else{
			$VhNumb = esc_html("Vehicle Number","wptopschool");

		}
		if(isset($item['DrName'])){
			$DrName = esc_html($item['DrName'],"wptopschool");
		}else{
			$DrName = esc_html("Driver Name","wptopschool");

		}
		if(isset($item['DrPhone'])){
			$DrPhone = esc_html($item['DrPhone'],"wptopschool");
		}else{
			$DrPhone = esc_html("Driver Phone","wptopschool");

		}
		if(isset($item['route_fees'])){
			$route_fees = esc_html($item['route_fees'],"wptopschool");
		}else{
		    $route_fees = esc_html("Route Fees","wptopschool");

		}
		if(isset($item['VhRoute'])){
			$VhRoute = esc_html($item['VhRoute'],"wptopschool");
		}else{
			$VhRoute = esc_html("Vehicle Route","wptopschool");
		...
		<div class="wpts-row">
					<div class="wpts-col-xs-12">
						<div class="wpts-panel-heading">
							<h3 class="wpts-panel-title">'.apply_filters('wpts_add_transportation_heading_item',esc_html__('Add Transportation', 'wptopschool' )).'</h3>
						</div>
							<div class="wpts-card-body">
							<div class="wpts-row">
										<div class="wpts-col-md-12">
											<div class="wpts-form-group">
												<label class="wpts-label" for="Name">'.$vname.'<span class="wpts-required">*</span></label>
												<input type="hidden"  id="wpts_locationginal" value='.esc_url(admin_url()).'"/>
												<input type="text" class="wpts-form-control" ID="VhName" name="VhName" placeholder="'.esc_attr($vname).'">
											</div>
										</div>
										<div class="wpts-col-md-6">
											<div class="wpts-form-group">
												<label class="wpts-label" for="VhNumber">'.$VhNumb.'<span class="wpts-required"> *</span></label>
												<input type="text" class="wpts-form-control select_date" ID="VhNumb" name="VhNumb" placeholder="'.esc_attr($VhNumb).'">
											</div>
										</div>
										<div class="wpts-col-md-6">
											<div class="wpts-form-group">
												<label class="wpts-label" for="Name">'.$DrName.'</label>
												<input type="text" class="wpts-form-control" ID="DrName" name="DrName" placeholder="Driver Name">
											</div>
										</div>
										<div class="wpts-col-md-6">
											<div class="wpts-form-group">
												<label class="wpts-label" for="Name">'.$DrPhone.'</label>
												<input type="text" class="wpts-form-control" ID="DrPhone" name="DrPhone" placeholder="'.esc_attr($DrPhone).'">
											</div>
										</div>
										<div class="wpts-col-md-6">
											<div class="wpts-form-group">
												<label class="wpts-label" for="Name">'.$route_fees.'</label>
												<input type="text" class="wpts-form-control" ID="route_fees" name="route_fees" placeholder="'.esc_attr($route_fees).'">
											</div>
										</div>
										<div class="wpts-col-md-12">
											<div class="wpts-form-group">
												<label class="wpts-label" for="Name">'.$VhRoute.'</label>
												<textarea name="VhRoute" class="wpts-form-control"></textarea>
											</div>
										</div>
										<div class="wpts-col-md-12">
												<button type="submit" id="TransSubmit" class="wpts-btn wpts-btn-success ">Submit</button>
		...
		echo wpts_kses_filter_allowed_html($form);
	}
	wp_die();
}
function wpts_UpdateTransport()
{
	wpts_Authenticate();
	global $wpdb;
	$trans_table = $wpdb->prefix . "wpts_transport";
	...
	else
	{
		...
		if (!empty($get_trans))
		{
			...
			$form = '<form name="TransEditForm" action="#" id="TransEditForm" method="post">
					<div class="wpts-row">
						...
						<div class="wpts-card">
							<div class="wpts-panel-heading">
								<h3 class="wpts-panel-title">Update Transport Information</h3>
							</div>
							<div class="wpts-card-body">
							<div class="wpts-row">
										<div class="wpts-col-md-12">
											<div class="wpts-form-group">
												...
												<input type="text" class="wpts-form-control" value="' . esc_attr($get_trans->bus_name) . '" name="VhName" placeholder="Vehicle Name">
											</div>
										</div>
										<div class="wpts-col-md-6">
											<div class="wpts-form-group">
												<label for="VhNumber">'.$VhNumb.'</label><span class="wpts-text-red">*</span>
												<input type="text" class="wpts-form-control select_date" value="' . esc_attr($get_trans->bus_no) . '" name="VhNumb" placeholder="'.esc_attr($VhNumb).'">
											</div>
										</div>
										<div class="wpts-col-md-6">
											<div class="wpts-form-group">
												<label for="Name">'.$DrName.'</label><span class="wpts-text-red">*</span>
												<input type="text" class="wpts-form-control" value="' . esc_attr($get_trans->driver_name) . '" name="DrName" placeholder="'.esc_attr($DrName).'">
											</div>
										</div>
										<div class="wpts-col-md-6">
											<div class="wpts-form-group">
												<label for="Name">'.$DrPhone.'</label><span class="wpts-text-red">*</span>
												<input type="text" class="wpts-form-control" value="' . esc_attr($get_trans->phone_no) . '" name="DrPhone" placeholder="'.esc_attr($DrPhone).'">
											</div>
										</div>
										<div class="wpts-col-md-6">
											<div class="wpts-form-group">
												<label for="Name">'.$route_fees.'</label>
												<input type="text" class="wpts-form-control" ID="route_fees" name="route_fees" placeholder="'.esc_attr($route_fees).'" value="' . esc_attr($get_trans->route_fees) . '">
											</div>
										</div>
										<div class="wpts-col-md-12">
											<div class="wpts-form-group">
												<label for="Name">'.$VhRoute.'</label><span class="wpts-text-red">*</span>
												<textarea name="VhRoute" class="wpts-form-control">' . esc_attr($get_trans->bus_route) . '</textarea>
											</div>
										</div>
									<div class="wpts-col-md-12">
									        <input type="hidden" name="transid" value="' . esc_attr($get_trans->id) . '">
											<button type="submit" class="wpts-btn wpts-btn-success" id="TransUpdate">Update</button>
		...
		{
			echo esc_html( 'Cant retrive data from DB!', 'wptopschool' );
		}
	}
	wp_die();
}
function wpts_ViewTransport()
{
	global $wpdb;
	$trans_table = $wpdb->prefix . "wpts_transport";
	$transid = intval($_GET['id']);
	...
	if (!empty($get_trans))
	{
		$content = '<div class="wpts-panel-heading">
						<h3 class="wpts-panel-title">Transport Information</h3>
					</div>
		<div class="wpts-panel-body">
			<div class="wpts-userDetails">
				<table class="wpts-table">
...
function wpts_DeleteTransport()
{
	...
	if ($del) echo esc_html( 'success', 'wptopschool' );
	wp_die();
}


function wpts_sendSubMessage(){
	...
	if (isset($_POST['main_m_id']) && !empty($_POST['main_m_id'])){
		...
		// do_action('wpts_message_updated', intval($mid) , $message_data);
		...
		if (!empty($subject) && !empty($r_id))
		{
			...
			{
				wpts_send_mail($receiverEmail, $subject, sanitize_text_field($_POST['message']));
			}
		}

		if ($send)
		{
			echo  esc_html( 'Message sent successfully!', 'wptopschool' );
		}
		// echo wpts_ViewMessage($mid, true);
	}
}



function wpts_SendMessage(){
	global $wpdb, $current_user;
	$messages_table = $wpdb->prefix . 'wpts_messages';
	...
	//	$delete_msg_count = $wpdb->get_var("select count(*) from $messages_delete_table where m_id = $mid  AND delete_status = 1");
		...
		if (!empty($subject) && !empty($receiverid))
		{
			...
			if (!empty($receiverEmail))
			{
				wpts_send_mail($receiverEmail, $subject, sanitize_text_field(stripslashes($_POST['message'])));
			}
		}
		// echo wpts_ViewMessage($mid, true);
	}
	else
	{
		if (isset($_POST['group']) && sanitize_text_field($_POST['group']) != '')
		{
			...
			if ($msg == '')
			{
				$send_error = esc_html("Message text Missing!", "wptopschool");
			}
			else
			{
				if($grp == 'admins'){
				  ...
				  $messages = isset($_POST['message']) ? sanitize_textarea_field($_POST['message']) : '';
					...
					foreach($receiver_list as $receiver)
					{
						if($receiver->ID != $sender){
							...
							do_action('wpts_message_created', $wpdb->insert_id, $message_data);
							$receiverEmail = isset($receiver->data->user_email) ? $receiver->data->user_email : '';
							if (!empty($receiverEmail))
							{
								wpts_send_mail($receiverEmail, $subj, $msg);
							}
						}
					}
				}
				else
				{
					...
					if ($explode[0] == 's')
					{
						...
						// $students_list_2 = $wpdb->get_results("SELECT DISTINCT wp_usr_id from $student_table WHERE sid in ( select DISTINCT st.sid from wp_wpts_student st JOIN $class_mapping_table cm on st.sid=cm.sid WHERE cid=$class_id)");
					...
					if(!empty($students_list)){
						...
						echo esc_html( 'Students not available. Please try again!', 'wptopschool' );
					}
					...
					if ($explode[0] == 'p')
					{
						...
						// $parent_list_2 = $wpdb->get_results("SELECT DISTINCT parent_wp_usr_id from $student_table WHERE sid in ( select DISTINCT st.sid from wp_wpts_student st JOIN $class_mapping_table cm on st.sid=cm.sid WHERE cid=$class_id)");
					...
					if (!empty($parent_list)){
						foreach($parent_list as $parent)
						{
							...
							echo  esc_html( 'Parent not available. Please try again!', 'wptopschool' );
						}
					}
					else
					if ($explode[0] == 't')
					{
						...
						if(!empty($teacher_list)){
							...
							echo esc_html( 'Teacher not available. Please try again!', 'wptopschool' );
						}
					}
					else
					{
						$send_error =  esc_html("Cannot determine group","wptopschool");
					}
				}
			}
		}
		else
		{
			if (!isset($_POST['r_id']))
			{
				echo esc_html( 'Please Select Receiver!', 'wptopschool' );
			}
			else
			{
				foreach(sanitize_price_array($_POST['r_id']) as $receiver)
				{
				  ...
				  $messages = isset($_POST['message']) ?  sanitize_textarea_field($_POST['message']) : '';
					...
					if (!empty($receiverEmail))
					{
						wpts_send_mail($receiverEmail, sanitize_text_field($_POST['subject']) , $_POST['message']);
					}
				}
			}
		}
...
function wpts_ViewMessage($mid = '', $return = false)
{
	global $wpdb, $current_user;
	if ($mid == '')
	{
	...
	$msgs_table = $wpdb->prefix . "wpts_messages";
	...
	// // die();
	// echo "</pre>";
	$change_stat = wpts_MarkRead($mid, $cuid);
	...
	$content = '<div class="wpts-row message_header">
            		<div class="wpts-col-md-12"><h3 class="card-title mt-5 mb-5"><strong>Subject : '. esc_html($get_mrow->subject) .'</strong></h3></div>
							</div>
						<div class="wpts-col-md-12" id="message_display"><ul>';

	$view_mb = wp_json_decode($get_mrow->msg);

	$content.= '<li><div class="wpts-row">
            <div class="wpts-col-md-1" style="float:left">'. esc_html(get_avatar($get_mrow->s_id, 50)) .'</div>
            <div class="wpts-col-md-11">
						<h3 style="margin-top:0;margin-bottom:5px;font-weight: 500;">'.esc_html($snickname).'<span class="m-date"> '.wpts_ViewDate(esc_html($get_mrow->m_date)).'</span></h3>
				...
				$content .='</li></ul>';
	//$content = apply_filters('wpts_message_content', $content, intval($mid));
	if ($return)
	{
		return $content;
	}
...
function view_sub_message($mid){
	...
	foreach ($get_mrow as $msg) {
		...
		$content .= '<ul><li><div class="wpts-row">
							<div class="wpts-col-md-1" style="float:left">
									'. esc_html(get_avatar($msg->r_id, 50)).'
							</div>
							<div class="wpts-col-md-11">
							<h3 style="margin-top:0;margin-bottom:5px;font-weight: 500;">'.esc_html($nickname).'<span class="m-date"> '.wpts_ViewDate(esc_html($msg->m_date)).'</span></h3>

							<p>'.esc_html($msg->msg).'</p>';
							if ($msg->s_id != $cuid ){
								$content .= '<a href="javascript:;" class="wpts-replay-message-btn" data-main_m_id="'.esc_attr($msg->main_m_id).'" data-replay_m_id="'.esc_attr($msg->mid).'" data-senderid="'.esc_attr($msg->r_id).'" data-reciver_id="'.esc_attr($msg->s_id).'">Reply</a>';
							}
...
function wpts_MarkRead($mid, $rid)
{
	global $wpdb;
	global $wpdb;
	$message_block = array();
	$table_name = $wpdb->prefix . 'wpts_messages';
...
function wpts_deleteMessage()
{
	...
	$cuid = intval($current_user->ID);
	$current_user_status = '';
	$msg_table = $wpdb->prefix . "wpts_messages";
	$msg_delete_table = $wpdb->prefix . "wpts_messages_delete";
...
function wpts_getTeachersList(){
	...
	if (isset($_POST['date']) && !empty($_POST['date'])){
		...
		$show_date = wpts_ViewDate($entry_date);
		$teacher_table = $wpdb->prefix . "wpts_teacher";
		$teacher_attendance_table = $wpdb->prefix . "wpts_teacher_attendance";
		...
		$title = _('New Attendance Entry', 'wptopschool');
		$allPresent = 0;
		if ($check_attend)
		{
			$title = __('Update Attendance Entry', 'wptopschool');
			$warning = __('Attendance already were entered!', 'wptopschool');
		...
		ob_start();
		echo '<div class="wpts-row"><div class="wpts-col-md-12">
						  ...
						  <form name="AttendanceEntryForm" id="AttendanceEntryForm" method="post" class="form-horizontal">
							...
							<div class="wpts-row">
								<div class="wpts-col-md-12">
								<input type="checkbox" class="ccheckbox checkAll wpts-checkbox" ' . checked($allPresent, 1, false) . ' name="Nil" value="Nil">
									<span class="wpts-text-green MRTen" style="vertical-align: middle;">All Present</span>
									</div>
									</div>';
									echo wp_nonce_field('StudentAttendance', 'sattendance_nonce', '', true) . '
								<div class="wpts-form-group">
								<table class="wpts-table wpts-table-bordered attendance-entry">
									<thead>
										<tr><td colspan="5"><span class="pull-right">Date: <span class="wpts-f500">' . esc_html($show_date) . '</span></span></td>
										</tr>
								...
								foreach($teachers as $st)
								{
									...
									echo '<tr><td>' . $sno . '</td>
												...
												<td><input type="checkbox" ' . checked($rchecked, 1, false) . ' class="ccheckbox wpts-checkbox" name="absent[]" value="' . esc_attr($st->wp_usr_id) . '"> Absent </td>
												<td><input type="text"  name="reason[' . esc_attr($st->wp_usr_id) . ']" value="' .esc_attr($reason) . '" class="wpts-form-control"></td>
											  </tr>';
									$sno++;
								}
							echo '</tbody>
		...
		echo wpts_kses_filter_allowed_html($content);
	}
	wp_die();
}
/* add/update teacher attendance */
function wpts_TeacherAttendanceEntry()
{
	if (!isset($_POST['sattendance_nonce']) || !wp_verify_nonce(sanitize_text_field($_POST['sattendance_nonce']) , 'StudentAttendance'))
	{
		echo esc_html( 'Unauthorized Submission', 'wptopschool' );
	...
	global $wpdb;
	$entry_date = date('Y-m-d', strtotime(sanitize_text_field($_POST['AttendanceDate'])));
	$att_table = $wpdb->prefix . "wpts_teacher_attendance";
	...
	if ($ins_attend)
	{
		$msg = esc_html( 'success1', 'wptopschool' );
	}
	else{
		$msg = esc_html( 'error', 'wptopschool' );
	}
...
function wpts_TeacherAttendanceView(){
	...
	if (isset($_POST['selectedate']) && !empty($_POST['selectedate']))
	{
		...
		{
			$result = esc_html('No Attendance entered yet...', 'wptopschool');
		}
		else
		{
			foreach($check_attend as $key => $value)
			...
			ob_start();
?> <table class="wpts-table" id="teacherAttendanceTable" cellspacing="0" width="100%" style="width:100%">
    <thead>
        <tr>
            <th><?php esc_html_e( 'Teacher Code', 'wptopschool' ); ?></th>
            <th><?php esc_html_e( 'Teacher Name', 'wptopschool' ); ?></th>
            <th><?php esc_html_e( 'Attendance', 'wptopschool' ); ?></th>
            <th><?php esc_html_e( 'Commment', 'wptopschool' ); ?></th>
        </tr>
    </thead>
    <tbody> <?php
            ...
            <td><?php
				if (isset($reasonList[$teacherInfo['wp_usr_id']])) esc_html_e( 'Absent', 'wptopschool' );
				else echo esc_html_e( 'Present', 'wptopschool' ); ?></td>
            <td><?php
	...
	{
		$result = esc_html('Please Select date','wptopschool');
	}
	echo wp_kses_post($result);
	wp_die();
}
/* remove attendance delete */
function wpts_TeacherAttendanceDelete(){
	wpts_Authenticate();
	global $wpdb;
...
function wpts_getNotifyInfo()
{
	...
	if (isset($_POST['notifyid']) && !empty($_POST['notifyid']))
	{
		...
		if (!empty($notifyInfo))
		{
			...
			$info = "<section class='wpts-content'>
						<div class='wpts-row'>
							<div class='wpts-col-xs-12 wpts-col-sm-12 wpts-col-md-12 wpts-col-lg-12'>
								<div class='wpts-panel panel-info'>
									<div class='wpts-panel-heading'>
										<h3 class='wpts-panel-title'>" . esc_html($notifyInfo->name) . "</h3>
									</div>
								<div class='wpts-panel-body'>
									<div class='wpts-row'>
										<div class='wpts-col-md-12 wpts-col-lg-12 '>
											<table class='wpts-table wpts-table-user-information'>
												<tbody>
													<tr>
														<td>" . __('Notification ID', 'wptopschool') . "</td>
														<td> ".esc_html($notifyInfo->nid)." </td>
													</tr>
													<tr>
														<td>" . __('Name', 'wptopschool') . "</td>
														<td> ".esc_html($notifyInfo->name)." </td>
													</tr>
													<tr>
														<td>" . __('Description', 'wptopschool') . "</td>
														<td> ".esc_html($notifyInfo->description)." </td>
													</tr>
													<tr>
														<td>" . __('Receiver', 'wptopschool') . "</td>
														<td> ".esc_html($receiver)." </td>
													</tr>
													<tr>
														<td>" . __('Notify Type', 'wptopschool') . "</td>
														<td> ".esc_html($type)." </td>
													</tr>
													<tr>
														<td>" . __('Notify Date', 'wptopschool') . "</td>
														<td> " . wpts_ViewDate($notifyInfo->date) . "</td>
													</tr>
												</tbody>
											</table>
										</div>
		...
		{
			$info = esc_html("No date retrived", 'wptopschool');
		}
	}
	echo wp_kses_post($info);
	wp_die();
}
function wpts_changepassword(){
	global $current_user;
	...
	if (wp_check_password($password, $current_user->data->user_pass, $current_user->ID)){
		if (sanitize_text_field($_POST['newpw']) == sanitize_text_field($_POST['newpw'])){
			...
			$msg = __('Password Updated Successfully', 'wptopschool');
			$status = 1;
		}
		else
		{
			$msg = __('New Password and re-enter New Password Should be same', 'wptopschool');
		}
	}
	else
	{
		$msg = __('Please enter correct old password', 'wptopschool');
	}
	$response = array(
		'status' => $status,
		'msg' => $msg
...
function wpts_getStudentsAttendanceList()
{
	...
	if (!empty($classID))
	{
		...
		if (count($leaveday) > 0)
		{
			$msg = __('<span class="wpts-text-red">N/A</span> Not Applicable(Date is marked as leave)', 'wptopschool');
		}
		else
		{
			...
			if (empty($attendance))
			{
				$msg = __('<span class="wpts-text-red">N/E</span> No Attendance Entered Yet', 'wptopschool');
			}
			...
			{
				ob_start();
				echo '<table class="wpts-table" id="attendanceOverview" cellspacing="0" width="100%" style="width:100%"><tr><th>' . __('Roll Number', 'wptopschool') . '</th>
							<th>' . __('Student Name', 'wptopschool') . '</th>
							<th>' . __('Attendance', 'wptopschool') . '</th>
							<th>' . __('Commment', 'wptopschool') . '</th>
							</tr>';
					...
					foreach($studentList as $key => $value)
					{
						...
						$sattendance = count($absentList) > 0 && array_key_exists($userID, $absentList) ? __('Absent', 'wptopschool') : __('Present', 'wptopschool');
			...
			elseif (empty($msg))
			{
				$msg = __('<span class="wpts-text-red">No Students Available in this class</span>', 'wptopschool');
			}
		}
		$title = '<h3 class="wpts-card-title">' . __('Attendance Overview', 'wptopschool') . '</h3>';
		$response['status'] = 1;
...
function wpts_listdashboardschedule(){
	global $wpdb, $current_user;
	$start = sanitize_text_field($_POST['start']);
	$end = sanitize_text_field($_POST['end']);
	$event_table = $wpdb->prefix . "wpts_events";
...
function wpts_Import_Dummy_contents(){
	global $wpdb;
	$wpts_teacher_table = $wpdb->prefix . "wpts_teacher";
	$wpts_class_table = $wpdb->prefix . "wpts_class";
	$wpts_student_table = $wpdb->prefix . "wpts_student";
	$wpts_subject_table = $wpdb->prefix . "wpts_subject";
	$teacherarray = $ins_teacher = $tch_ins_arr = $wpts_class_ins = $studentarray = $sp_stu_ins = array();
	...
	foreach($teacherarray as $key => $value){
		$userInfo = array(
			...
			'user_email' => $value['first_name'] . 'wpts@yourdomain.com',
			'role' => 'teacher'
		);
		$user_id = wp_insert_user($userInfo);
		if (!is_wp_error($user_id)){
			$value['wp_usr_id'] = $user_id;
			$tch_ins = $wpdb->insert($wpts_teacher_table, $value);
			$tch_ins_arr[] = $user_id;
		}
	}
	if (count($tch_ins_arr) > 0){
		...
		foreach($tch_ins_arr as $key => $value){
			...
			if($i <= 3){
			$wpdb->insert($wpts_class_table, $classarray[$key]);
			$wpts_class_ins[] = $wpdb->insert_id;
			}
			else {
			}
			$i++;
	...
	if (count($tch_ins_arr) > 0){
		...
		foreach($tch_ins_arr as $key => $value){
			...
			if($i <= 9){
			$wpdb->insert($wpts_subject_table,$subjectarray[$key]);
			$wpts_subject_ins[] = $wpdb->insert_id;
			}
			else {
			}
			$i++;
	...
	if (count($wpts_class_ins) > 0){
		foreach($studentarray as $key => $value){
			...
			$userInfo = array(
				...
				'user_email' => $value['p_fname'] . 'wpts@yourdomain.com',
				'role' => 'parent'
			);
			...
			if (!is_wp_error($student_id) && !is_wp_error($parent_id)){
				...
				$value['class_id'] = $wpts_class_ins[$key];
				$wpdb->insert($wpts_student_table, $value);
