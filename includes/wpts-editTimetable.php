		...
		<div class="wpts-col-md-12">
			<div class="wpts-card">
				...
				<div class="wpts-card-body">
					...
					if( count($clt) == 0 ) {
						$error = 1;
						echo "<div class='wpts-text-red'>No Subjects retrieved, Check you have subject for this class at <a href='".esc_url(site_url()."/wp-admin/admin.php?page=sch-subject")."'>Subjects</a></div>";
					}
					if( $error == 0 ) {
						$timetable	=	array();
						...
						<div class="wpts-row">
							<div class="wpts-col-md-12">
								<div class="wpts-form-group">
									<label class="wpts-labelMain">
									<?php esc_html_e("Class","wptopschool");?> :  <?php echo wpts_GetClassName( $class_id ); ?>
									</label>
								</div>
							</div>
						</div>
								...
								<thead>
									<tr><th><!-- <select class="daytype"><option value="0">Days</option><option value="1"> -->Week<!-- </option></select> --></th>
										<?php foreach ($session as $sess) { ?>
											<th><?php  $sess = esc_sql($sess); $ses_info = $wpdb->get_row("Select * from $wpts_hours_table where id='$sess'");
                ...
                //                                print_r($sub_name_f);
												...
												if( !empty( $sub_name ) ) {
													if($td_class == "break"){
													$sub_name	=	'<div class="item1 assigned wpts-assigned-item">Break<a href="javascript:void(0)" class="" data-id='. esc_attr($key).' data-rowid='.esc_attr($j).'  ></a></div>';
													} else {
 														if($subida1[$pa] == 1){
                                                $sub_name = '-';
                                                    } else {
                                                        $sub_name	=	'<div class="item1 assigned wpts-assigned-item">'.esc_attr($sub_name).'<a class="daleteid wpts-tt-delete-icon" data-id='. esc_attr($key).' data-rowid='.esc_attr($j).'  ></a></div>';
                                                    }
