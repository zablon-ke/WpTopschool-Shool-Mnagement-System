$(document).ready(function() {
  $(".multiselect").change(function() {
    ...
    $('.multiselect').closest(".wpts-col-md-12").find('.date-input-block table').html('');
    jQuery.each(classarray, function(i, val) {
      $('.multiselect').closest(".wpts-col-md-12").find('.date-input-block table').append("<tr><td style='border:1px solid;padding:5px;'><strong id='" + val['text'] + "'>" + val['text'] + "</strong></td><td style='border:1px solid;padding:5px;'><input type='text' class='someclass datepicker' name='Classdata[]' placeholder='yyyy-mm-dd'></td></tr>");
    });
  });
    ...
    submitHandler: function(form) {
      ...
      $.ajax({
        ...
        success: function(rdata) {
          ...
          if (rdata == 'success') {
            $(".wpts-popup-return-data").html('Student added successfully !');
            $("#SuccessModal").css("display", "block");
            $("#SavingModal").css("display", "none");
            $("#SuccessModal").addClass("wpts-popVisible");
            var wpts_pageURL = $('#wpts_locationginal1').val();
            var delay = 1000;
            var url = wpts_pageURL + "admin.php?page=sch-student";
            ...
            $(".wpts-popup-return-data").html(rdata);
            $("#SavingModal").css("display", "none");
            $("#WarningModal").css("display", "block");
            $("#WarningModal").addClass("wpts-popVisible");
          }
        },
        error: function() {
          $("#SavingModal").css("display", "none");
      ...
      $.ajax({
        ...
        success: function(rdata) {
          ...
          if (rdata == 'success0') {
            ...
            $("#SavingModal").css("display", "none");
            $("#SuccessModal").addClass("wpts-popVisible");
            var wpts_pageURL = $('#wpts_locationginal').val();
            var delay = 1000;
            var url = wpts_pageURL + "admin.php?page=sch-student";
            var timeoutID = setTimeout(function() {
              window.location.href = url;
    ...
    if (singledata == 'true'){
      ...
      jQuery.post(ajax_url, data, function(cddata) {
        if (cddata == 'success') {
          ...
          $(".wpts-popup-return-data").html('Operation failed.Something went wrong!');
          $("#SavingModal").css("display", "none");
          $("#WarningModal").css("display", "block");
          $("#WarningModal").addClass("wpts-popVisible");
        }
  ...
  $('#bulkaction').change(function() {
    ...
    if (op == 'bulkUsersDelete') {
      ...
      if (uids.length == 0) {
        $(".wpts-popup-return-data").html('No user selected!');
        $("#SavingModal").css("display", "none");
        $("#WarningModal").css("display", "block");
        $("#WarningModal").addClass("wpts-popVisible");
        return false;
  ...
  if ($(window).width() < 991) { //alert('test');
    $('#verticalTab').find('.wpts-resp-tab-active').removeClass('wpts-resp-tab-active');
    $('#verticalTab').find('.wpts-resp-tab-content-active').removeClass('wpts-resp-tab-content-active').css("display", "");
  }
  if (jQuery(window).width() < 991) {
    //alert('test');
    if (!$('#verticalTab').find('.wpts-resp-tab-active').length) {
      //alert('test');
      $('.wpts-resp-tabs-list .wpts-resp-tab-item:first-child').click();
    }
  }
});
