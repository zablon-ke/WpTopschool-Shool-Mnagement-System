$(document).ready(function($) {
    ...
    $(document).on("click", ".wpts-replay-message-btn", function() {
        ...
        var html = '<form name="replySubMessage" action="javascript:;" class="replySubMessageForm" method="post">' +
            ...
            '<textarea id="message" name="message" style="width:100%;height:100%;background:transparent" class="wpts-form-control" placeholder="Enter Message"></textarea></div></br><span class="form-group">' +
            '<input type="submit" class="wpts-btn wpts-btn-success" class="sendSubReply" style="margin:10px" value="SEND">' +
    ...
    $(document).on("click", "#createMessage", function() {
        ...
        $('#' + linkAttrib).addClass("wpts-popVisible");
        $('body').addClass('wpts-bodyFixed')
    });
    ...
    $(document).on('click', '.msg-child-list', function(e) {
        ...
        $('.wptss-message-list').removeClass('none');
        $('.wp-subject-list').addClass('none');
        if ($('#student_teacher').is(':checked')) {
            $('.wp-subject-list').addClass('none');
            var classid = $('input[name=childname]:checked').attr('classid');
