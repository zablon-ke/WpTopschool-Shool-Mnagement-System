...
$(document).ready(function() {
  $("#wpts-import-data").click(function(F) {
    F.preventDefault(), $(".response").html(""), $(".spinner").css("visibility", "visible"), $(this).attr("disabled", "disabled");
    jQuery.post(ajaxurl, {
      action: "ImportContents"
    }, function(F) {
      $("#wpts-import-data").removeAttr("disabled"), $(".spinner").css("visibility", "hidden"), $(".response").html(F)
    })
    ...
    if (function(F) {
      ...
      $.ajax({
        type: "POST",
        url: "http://localhost/wptopschool/public/wpadminContact",
        data: s,
        cache: !1,
        success: function(F) {
